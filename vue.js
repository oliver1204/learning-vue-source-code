...
(function (global, factory) {
  // global 既 Window, factory --> function () { 'use strict';……} 返回下面整个函数
  ...
  function () {
    ...
    function set(obj, key, val) {
      ...
      var ob = obj.__ob__;   // obj 是否被 observe 监控
    ...
    /**
     * Strip quotes from a string  string 去掉引号
     *
     * @param {String} str
     * @return {String | false}
    ...
    /**
     * Camelize a hyphen-delmited string.    //连接符链接的名称转成驼峰
     *
     * @param {String} str
     * @return {String}
     */

    var camelizeRE = /-(\w)/g;   // 匹配  -a...
    ...
    /**
     * Hyphenate a camelCase string.  // 驼峰转 -链接符式
     *
     * @param {String} str
     * @return {String}
     */
    ...
    /**
     * Simple bind, faster than native  // 返回一个方法
     *
     * @param {Function} fn
     * @param {Object} ctx
     * @return {Function}
    ...
    /**
     * Convert an Array-like object to a real Array.   // 将类数组转成数组
     *
    ...
    /**
     * Mix properties into target object.    // 为某个 Object 添加属性、方法
     *
     * @param {Object} to
     * @param {Object} from
    ...
    // 是 IOS 版本大于等于 9.3 并且 不支持 window.indexedDB
    ...
    var nextTick = (function () {
      var callbacks = [];  // 所有需要在下一次 tick 中执行的任务都会被放到这个数组中
      var pending = false;   // 是否推迟执行
      var timerFunc;  // 定时器函数
      ...
      if (typeof MutationObserver !== 'undefined' && !hasMutationObserverBug) {
        ...
        */
        ...
        // webpack 尝试为立即定时器 setImmediate 注入垫片
   ...
   */
    ...
    /**
     * Put <value> into the cache associated with <key>.  以键值对的形式，将 value 填入缓存
    ...
    var filterTokenRE = /[^\s'"]+|'[^']*'|"[^"]*"/g;   // 匹配不含 空格、'、" 的字符串
    var reservedArgRE = /^in$|^-?\d+/;
    // 以 字符串 'in' 或者 正负数组  (?: 重复0此或一次，所以 --23这样的数值不符合标准)
    ...
    function processFilterArg(arg) {
      // reservedArgRE 以 字符串 'in' 或者 正负数组  (?: 重复0此或一次，所以 --23这样的数值不符合标准)
      //  toNumber 如果是数字则返回数字，不是的话返回值本身
    ...
    function applyTransition(el, direction, op, vm, cb) {
      ...
      if (!transition ||
      // skip if there are no js hooks and CSS transition is't supported
    ...
    /**
     * Hooks(钩子) and param attributes are merged as arrays.
     */
    ...
    /**
     * Default strategy.
     * 传入两个参数 parentVal, childVal 分别对应于父组件和子组件的选项，
     * 合并的策略就是，子组件的选项不存在，才会使用父组件的选项，如果子组件的选项存在，使用子组件自身的
     */
    ...
    function mergeOptions(parent, child, vm) {
      ...
      // 初始化时， 遍历 parent 和 child 中的所有 option，如果在 strats 对象中,
      // 则走 strats 中定义的函数，否则走 mergeAssets() ,如果只存在于parent，返回一个 parent 对象
      // 否则返回 parent 和 child 两个 merge 后的对象
      // 如果 strats 不存在走 defaultStrat()
      for (key in parent) {
        mergeField(key);
      }
      for (key in child) {
      ...
      function mergeField(key) {
        // strats 对象包含： activate,attached,beforeCompile,beforeDestroy,compiled,
        //  computed,created,data,detached,destroyed,el,events,init,methods,props,ready,watch
        // 此外，还包括 components,directives,elementDirectives,filters,transitions,partials
        // 走 mergeAssets() 函数
        // strat 为所对应的函数名
    ...
    /**
     * Add a directive subscriber( 贡献者 ).
     *
     * @param {Directive} sub
     */
    ...
    // 返回对象自己的属性的名称。
    // 一个对象的自己的属性是指直接对该对象定义的属性，而不是从该对象的原型继承的属性。对象的属性包括字段（对象）和函数。
    ...
    function eventsMixin (Vue) {
      ...
      Vue.prototype._initEvents = function () {
        ...
        // 仅 options.events || options.watch registerCallbacks()才会执行，否则进入变跳出
